#!/usr/bin/env python3
"""Step 1b builder for lect-05b-all_scripting-animation: converts the
extracted JSON into the standard lecture slide shell (see
docs/lecture-conversion-playbook.md section 8a). Same conventions as
temp/build_lect_04b.py: heading=first line -> h2, lines ending ":" -> h3,
short ALL-CAPS -> section-label, bullet lines -> ul/li, "From <url>" ->
slide-citation link, all text HTML-escaped, vertical-tab (\\x0b) soft breaks
...
def render_lines(lines):
    ...
    for line in lines:
        ...
        if line.endswith(":") and len(line) < 70:
            ...
            out.append(f'    <p class="section-label">{escaped}</p>')
        else:
